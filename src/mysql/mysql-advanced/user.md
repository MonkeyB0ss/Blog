...
CREATE USER 'monkeyboss'@'localhost' IDENTIFIED BY '123456';
```
...
DROP USER 'monkeyboss'@'localhost';
```
...
- `FILE权限`使用户可以使用LOAD DATA INFILE和SELECT ... INTO
...
- 给li4用户用本地命令行方式，授予monkeyboss这个库下的所有表的插删改查的权限。

```sql
GRANT SELECT,INSERT,DELETE,UPDATE ON monkeyboss.* TO li4@localhost;
```
...
```sql
SET DEFAULT ROLE ALL TO 'monkeyboss'@'localhost';
```
